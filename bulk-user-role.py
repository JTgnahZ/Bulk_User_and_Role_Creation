...
#Prisma SDWAN script to read from csv and bulk create tenant operator or update operator roles.
#tiazhang@paloaltonetworks.com
...
#Creating system role and customer role maps
tenant_roles = []
tenant_rolemap = {}
...
with open(bulkcsv, 'r') as csv_file:
    ...
    for data in csv_reader:
        ...
        #Parse and split role dict if necessary
        if roles:
            if "," in roles:
                tmp = roles.split(",")
                ...
                for role in tmp:
                    if role not in tenant_roles:
                        if role not in custom_roles:
                            ...
                            update_data["custom_roles"].append(mappedrole)
                                
                    else:
                        mappedrole = tenant_rolemap[role]
                        signup_data["roles"].append(mappedrole)
                        update_data["roles"].append(mappedrole)
            else:
        ...
        #If email exsits in CSV
        #-Check if email address already exist in system, if yes, update user roles only
        #-If email is not existed in system, sign-up a new user
        #If email doesn't exist in CSV, display error message and exit script.
        if email:
            if email in email_list:
                #Retrieve operator ID and etag
                resp = cgx_session.get.tenant_operators()
                if resp.cgx_status:
                    existing_users = resp.cgx_content.get("items",None)
                    for user in existing_users:
                        if email in user.values():
                            oid = user["id"]
                            update_data.update({"_etag" : user["_etag"]})
                            pp.pprint(oid)
                            pp.pprint(update_data)
                            resp = cgx_session.patch.tenant_operators(operator_id = oid, data=update_data)
                            cloudgenix.jd_detailed(resp)
            else:
